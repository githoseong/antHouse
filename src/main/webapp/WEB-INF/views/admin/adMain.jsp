...
data.addRow(['총방문수', ${listVO[0].totalSum}, ${listVO[0].totalCnt}]); 
data.addRow(['오늘방문수', ${listVO[0].todaySum}, ${listVO[0].todayMax}]); 
data.addRow(['1주일이내', ${listVO[0].weekCnt}, 1]); 
				...
				<div id="memPhoto">
					...
					<div style="height:80%;width:90%;margin:10px auto auto; border:1px dotted;">
						<div id="one">
							...
							<div class="rank" id="one1"><img src="${ctp}/member/${listVO[0].photo}" class="rankimg" ></div>
						</div>
						<div class="two" id="two">
							<i class="fa-solid fa-crown" style="color:silver"></i>
							<div class="rank" style="width:60%;"  id="two2"><img src="${ctp}/member/${listVO[1].photo}" class="rankimg"/></div>
						</div>
						<div class="two" id="three">
							<i class="fa-solid fa-crown" style="color:brown"></i>
							<div class="rank" style="width:60%;" id="three3"><img src="${ctp}/member/${listVO[2].photo}" class="rankimg" /></div>
						</div>
					</div>
				</div>
				<div id="memtext" class="memtext">
					<strong>아이디</strong> : ${listVO[0].mid} <br/>
					<strong>이름</strong> : ${listVO[0].name}<br/>
					<strong>이메일</strong> : ${listVO[0].mail}<br/>
					<strong>전화번호</strong> : ${listVO[0].tel}<br/>
					<strong>총 접속일</strong> : ${listVO[0].totalCnt}<br/>
					<strong>최근 방문</strong> : ${listVO[0].lastDate}<br/>
					<strong>게시물</strong> : ${bVo.boardCnt} 개<br/>
					<strong>댓글 수</strong> : ${bVo.boardreplyCnt} 개<br/>
				</div>
				<div id="memtext2" class="memtext">
					<strong>아이디</strong> : ${listVO[1].mid} <br/>
					<strong>이름</strong> : ${listVO[1].name}<br/>
					<strong>이메일</strong> : ${listVO[1].mail}<br/>
					<strong>전화번호</strong> : ${listVO[1].tel}<br/>
					<strong>총 접속일</strong> : ${listVO[1].totalCnt}<br/>
					<strong>최근 방문</strong> : ${listVO[1].lastDate}<br/>
					<strong>게시물</strong> : ${bVo2.boardCnt} 개<br/>
					<strong>댓글 수</strong> : ${bVo2.boardreplyCnt} 개<br/>
				</div>
				<div id="memtext3" class="memtext">
					<strong>아이디</strong> : ${listVO[2].mid} <br/>
					<strong>이름</strong> : ${listVO[2].name}<br/>
					<strong>이메일</strong> : ${listVO[2].mail}<br/>
					<strong>전화번호</strong> : ${listVO[2].tel}<br/>
					<strong>총 접속일</strong> : ${listVO[2].totalCnt}<br/>
					<strong>최근 방문</strong> : ${listVO[2].lastDate}<br/>
					<strong>게시물</strong> : ${bVo3.boardCnt} 개<br/>
					<strong>댓글 수</strong> : ${bVo3.boardreplyCnt} 개<br/>
				</div>
			</div>
			<div id="visit">
				...
				<div id="visit2">
					<div class="visit3">
						<div>총 접속</div>
						<div class="visitNum">${listVO[0].totalSum}</div>
					</div>
					<div class="visit3">
						<div>오늘 접속</div>
						<div class="visitNum">${listVO[0].todaySum}</div>
					</div>
					<div class="visit3">
						<div>1주일 이내 접속</div>
						<div class="visitNum">${listVO[0].weekCnt}</div> 
					</div>
				</div>
			</div>
		<div id="member">
